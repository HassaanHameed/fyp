import React from "react";

const BlogPage = () => {
  return (
    <>
      <section
        id="blog-page-hero"
        style={{ background: "url(img/blog/blog-hero1.jpg)" }}
      >
        <div className="container">
          <div className="row">
            <div className="col-lg-12 col-md-12 col-sm-12">
              <div className="blog-hero-area">
                <div className="blog-category-title">
                  <h2>
                    Category archive for : <span>Marketing</span>
                  </h2>
                </div>
                <div className="blog-post-meta">
                  <div className="left-meta">
                    <ul>
                      <li>
                        <span>
                          <i className="far fa-dot-circle" />
                        </span>{" "}
                        Trend
                      </li>
                      <li>
                        <span>
                          <i className="far fa-dot-circle" />
                        </span>{" "}
                        by Rafsan Jany
                      </li>
                    </ul>
                  </div>
                  <div className="right-meta">
                    <ul>
                      <li>
                        <span>
                          <i className="far fa-dot-circle" />
                        </span>{" "}
                        23, Dec. 2018
                      </li>
                      <li>
                        <span>
                          <i className="far fa-dot-circle" />
                        </span>{" "}
                        No Comments
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
      <section id="blog-grid">
        <div className="container">
          <div className="row">
            <div className="col-lg-8 col-md-8 col-sm-12">
              <div className="row">
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-4.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Jan. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            ios app
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">
                          Pros &amp; Cons of Premined App Instaling
                        </a>
                      </div>
                      <div className="text">
                        <p>
                          How to install the app with in 2 munites. Most easiest
                          way ever. How to install the app with in 2 munites.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-1.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Nov. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            Fashion
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">Style Icons Menswear Muses for Winter</a>
                      </div>
                      <div className="text">
                        <p>
                          We always cue up music when we’re entertaining. But
                          when it’s just us puttering around the kitchen
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-5.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Jan. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            ios app
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">
                          Pros &amp; Cons of Premined App Instaling
                        </a>
                      </div>
                      <div className="text">
                        <p>
                          How to install the app with in 2 munites. Most easiest
                          way ever. How to install the app with in 2 munites.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-6.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Nov. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            Travel
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">
                          Reintroducing Our Favorite Anthropologie Prints
                        </a>
                      </div>
                      <div className="text">
                        <p>
                          We always cue up music when we’re entertaining. But
                          when it’s just us puttering around the kitchen
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-2.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Nov. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            Music
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">Athena Calderone’s Tasting Notes</a>
                      </div>
                      <div className="text">
                        <p>
                          How to install the app with in 2 munites. Most easiest
                          way ever. How to install the app with in 2 munites.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-7.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Nov. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            Travel
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">
                          Reintroducing Our Favorite Anthropologie Prints
                        </a>
                      </div>
                      <div className="text">
                        <p>
                          We always cue up music when we’re entertaining. But
                          when it’s just us puttering around the kitchen
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-3.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Nov. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            Music
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">Have you Heard? Top Tunes of 2014</a>
                      </div>
                      <div className="text">
                        <p>
                          If you’ve ever visited an Anthropologie store, you’re
                          likely familiar with the work of Katie
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-6 col-md-6 col-sm-12">
                  <div className="blog-posts">
                    <div className="blog-image">
                      <img src="img/blog/blog-8.jpg" alt="" />
                    </div>
                    <div className="blog-details">
                      <ul className="post-meta">
                        <li>Nov. 05. 2019</li>
                        <li>
                          <a href="#" className="tags">
                            Fashion
                          </a>
                        </li>
                      </ul>
                      <div className="blog-title">
                        <a href="#">
                          From our speakers to yours, happy listening!
                        </a>
                      </div>
                      <div className="text">
                        <p>
                          So enamored are we of the master mood-setter’s
                          pitch-perfect playlists Lorem ipsum dolor.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="col-lg-12 col-md-12 col-sm-12">
                  <div className="pagination-area">
                    <ul>
                      <li>
                        <a href="#">
                          <i className="fas fa-chevron-left" />
                        </a>
                      </li>
                      <li>
                        <a href="#">1</a>
                      </li>
                      <li>
                        <a href="#">2</a>
                      </li>
                      <li>
                        <a href="#">3</a>
                      </li>
                      <li>
                        <a href="#">4</a>
                      </li>
                      <li>
                        <a href="#">5</a>
                      </li>
                      <li>
                        <a href="#">
                          <i className="fas fa-chevron-right" />
                        </a>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <div className="col-lg-4 col-md-4 col-sm-12">
              <div className="blog-sidebar">
                <div className="widget search-widget">
                  <div className="widget-title">
                    <h4>Search</h4>
                  </div>
                  <div className="search-field">
                    <form className="search-form">
                      <input type="text" placeholder="Search" />
                      <button className="search-btn">
                        <i className="fas fa-search" />
                      </button>
                    </form>
                  </div>
                </div>
                <div className="widget category-widget">
                  <div className="widget-title">
                    <h4>Categories</h4>
                  </div>
                  <ul className="categories">
                    <li>
                      <a href="#">Bloggers</a>
                    </li>
                    <li>
                      <a href="#">Designer</a>
                    </li>
                    <li>
                      <a href="#">Development</a>
                    </li>
                    <li>
                      <a href="#">News</a>
                    </li>
                  </ul>
                </div>
                <div className="widget recent-post-widget">
                  <div className="widget-title">
                    <h4>Recent Post</h4>
                  </div>
                  <div className="recent-posts">
                    <div className="feature-image">
                      <a href="#">
                        <img src="img/blog/recent-1.jpg" alt="" />
                      </a>
                    </div>
                    <div className="post-details">
                      <h6 className="post-title">
                        <a href="#">How to install the app with in 2 munites</a>
                      </h6>
                      <div className="post-meta">
                        <ul>
                          <li className="date">01 April 2019</li>
                          <li>
                            <i className="far fa-heart" /> 135
                          </li>
                          <li>
                            <i className="far fa-comment" /> 35
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div className="recent-posts">
                    <div className="feature-image">
                      <a href="#">
                        <img src="img/blog/recent-2.jpg" alt="" />
                      </a>
                    </div>
                    <div className="post-details">
                      <h6 className="post-title">
                        <a href="#">Style your own workspace with this app</a>
                      </h6>
                      <div className="post-meta">
                        <ul>
                          <li className="date">01 April 2019</li>
                          <li>
                            <i className="far fa-heart" /> 120
                          </li>
                          <li>
                            <i className="far fa-comment" /> 21
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div className="recent-posts">
                    <div className="feature-image">
                      <a href="#">
                        <img src="img/blog/recent-3.jpg" alt="" />
                      </a>
                    </div>
                    <div className="post-details">
                      <h6 className="post-title">
                        <a href="#">Best 10 tips for this app lorem ipsum</a>
                      </h6>
                      <div className="post-meta">
                        <ul>
                          <li className="date">03 April 2019</li>
                          <li>
                            <i className="far fa-heart" /> 103
                          </li>
                          <li>
                            <i className="far fa-comment" /> 07
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="widget comments-widget">
                  <div className="widget-title">
                    <h4>Recent Comments</h4>
                  </div>
                  <div className="comment">
                    <span className="icon">
                      <i className="far fa-comment" />
                    </span>
                    <p>
                      <a href="#" className="name">
                        David Willson
                      </a>{" "}
                      <i>on</i> Building a New Successful Hostel Campaign
                    </p>
                  </div>
                  <div className="comment">
                    <span className="icon">
                      <i className="far fa-comment" />
                    </span>
                    <p>
                      <a href="#" className="name">
                        Charli Brown
                      </a>{" "}
                      <i>on</i> Building a New Successful Hostel Campaign
                    </p>
                  </div>
                </div>
                <div className="widget tag-widget">
                  <div className="widget-title">
                    <h4>Tags</h4>
                  </div>
                  <ul className="tags">
                    <li>
                      <a href="#">providing</a>
                    </li>
                    <li>
                      <a href="#">Spend</a>
                    </li>
                    <li>
                      <a href="#">Value</a>
                    </li>
                    <li>
                      <a href="#">Spend</a>
                    </li>
                    <li>
                      <a href="#">Value</a>
                    </li>
                    <li>
                      <a href="#">Spend</a>
                    </li>
                    <li>
                      <a href="#">providing</a>
                    </li>
                    <li>
                      <a href="#">Spend</a>
                    </li>
                    <li>
                      <a href="#">providing</a>
                    </li>
                    <li>
                      <a href="#">Spend</a>
                    </li>
                    <li>
                      <a href="#">Value</a>
                    </li>
                    <li>
                      <a href="#">Spend</a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
};

export default BlogPage;
